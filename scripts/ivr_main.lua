	...
	local function constructApi(methodType,contentType,serviceURL,apiInputdata)
		...
		for _, item in ipairs(apiInputdata) do
			...
			elseif item.InputType == "R" then
				if item.InputValueType == "D" or item.InputValueType == "E" then
					 freeswitch.consoleLog("INFO", "IFpart: " .. item.InputType)
					payload[item.FieldName] = session:getVariable(item.InputValue)
				else
					freeswitch.consoleLog("INFO", "Elsepart: " .. item.InputType)
...
--		 session:setVariable("CallerNameTextEn", "please_get_from_recording")
  --  session:setVariable("IncidentDetailsTextEn", "please_get_from_recording")
...
local function operation_code_341_exec(data) -- Speech to Text
    ...
    freeswitch.consoleLog("INFO", "Audio file to convert into Text: " .. tostring(audioFile) .. "\n")

--    local languageCode = session:getVariable("LanguageCode")
session:setVariable("Language_Code", "zz-ZZ")
  
freeswitch.consoleLog("INFO", "Language Code: " .. tostring(languageCode) .. "\n")

    local api_id = 20
    local methodType, contentType, serviceURL, apiInputdata, apiOutput
    ...
    local finalApi = constructApi(methodType, contentType, serviceURL, apiInputdata)
    freeswitch.consoleLog("NOTICE", "Final modified API: " .. tostring(finalApi) .. "\n")
finalApi = finalApi .. " --connect-timeout 5 --max-time 7"
    -- ---------------- STT API Call ----------------
    local apicallResult = apiCall(contentType, finalApi, apiOutput)

    -- ---------------- Wait for STT response (max 5 sec) ----------------
    local max_wait = 5
    local waited = 0
    local stt_ready = false

    while waited < max_wait do
        if apicallResult and apicallResult ~= "" then
            stt_ready = true
            break
        end
        freeswitch.consoleLog("WARNING", "STT response delayed... waiting 1 sec\n")
        freeswitch.msleep(1000)
        waited = waited + 1
    end

    -- ---------------- Fallback if STT delayed/failed ----------------
    if not stt_ready then
        freeswitch.consoleLog("ERROR", "STT timeout. Using hardcoded fallback values.\n")

        session:setVariable("CallerNameTextEn", "IVR")
        session:setVariable("IncidentDetailsTextEn", "Listen_from_recording")

        -- Force DTMF = S
        data.DtmfInput = "S"
        find_childNode_with_dtmfinput(nil, data)
        return
    end
    ...
    for _, settings in pairs(generalSettings) do
        if settings.SettingId == 14 then
            ...
            session:setVariable(
                data.TagName,
                session:getVariable(stt_settingValue.TextResponseFieldTag)
            )
            break
        end
    end
...
local function operation_code_341_exec(data) -- Speech to Text
    ...
    -- ================================
    -- STEP 2: Fire STT API (blocking)
    -- ================================
    local apicallResult = apiCall(contentType, finalApi, apiOutput)

    -- ================================
    -- STEP 3: WAIT FOR STT RESULT (MAX 4s)
    -- ================================
    local stt_value = nil
    ...
    while waited < max_wait_ms do
        for _, settings in pairs(generalSettings) do
            if settings.SettingId == 14 then
                local stt_settingValue = json.decode(settings.SettingValue)
                stt_value = session:getVariable(stt_settingValue.TextResponseFieldTag)
                break
            end
        end

        if stt_value and stt_value ~= "" then
            freeswitch.consoleLog("INFO", "STT received: " .. stt_value .. "\n")
            break
        end
    ...
    -- ================================
    -- STEP 4: FALLBACK IF TIMEOUT
    -- ================================
    if not stt_value or stt_value == "" then
        freeswitch.consoleLog("WARNING", "STT timeout exceeded, using fallback values\n")

        session:setVariable("CallerNameTextAr", "IVR")
        session:setVariable("IncidentDetailsTextAr", "please_get_from_recording")
        session:setVariable("stt_status", "timeout")
	 data.DtmfInput = "S"

    	find_childNode_with_dtmfinput("S", data)
	 return
    else
        -- Remove quotes if present
        stt_value = stt_value:gsub('^"', ''):gsub('"$', '')
        session:setVariable(data.TagName, stt_value)
        session:setVariable("stt_status", "success")
       
    end

    -- ================================
    -- STEP 5: CONTINUE IVR FLOW
    -- ================================
	 data.DtmfInput = "S"
