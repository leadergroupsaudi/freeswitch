    ...
    try:
        ...
        if token_header:
            ...
            if 'httpStatusCode' in resp.keys() and resp['httpStatusCode'] == 'OK':
                logger.debug("Successfully uploaded compressed recording file: {}".format(compressed_file_path))
               # os.remove(compressed_file_path)  # Delete the compressed file after successful upload
            else:
                logger.debug("Failed to upload audio recording file: {}".format(compressed_file_path))
...
def subscribe_to_events(conn):
    try:
        # Subscribe to the PRESENCE_IN event after reconnecting
        conn.send("event PRESENCE_IN")
        logger.debug("Subscribed to PRESENCE_IN events.")
    except Exception as e:
        logger.error(f"Failed to subscribe to events: {str(e)}")

@app.handle("PRESENCE_IN")
async def presence_in_event_handler(event):
    try:
        logger.debug(f"Received PRESENCE_IN event: {event}")
        # Extract agent extension and presence status from the event
        agent_extension = event.get("from").split('@')[0]
        presence_status = event.get("status")  # Example: "Available" or "DND"

        logger.debug(f"Agent {agent_extension} Presence Status: {presence_status}")

        # Use the existing connect_to_fs function to get the connection
        conn = connect_to_fs()
        if conn.connected():
            # Set the agent's status as a global variable in FreeSWITCH
            if presence_status == "Busy":
                conn.api(f"global_setvar agent_{agent_extension}_status=Busy")
                logger.debug("in if condition")
            else:
                conn.api(f"global_setvar agent_{agent_extension}_status=Available")

            logger.debug(f"Updated Agent {agent_extension} status to {presence_status} in FreeSWITCH")

            # Ensure resubscription after handling the event
        ...
        logger.error(f"Error in PRESENCE_IN event handler: {str(e)}")
...
async def keep_alive(conn):
    while True:
        try:
            logger.debug("Sending keep-alive to FreeSWITCH...")
            conn.api("status")  # Ping the server to keep the connection alive
            await asyncio.sleep(60)  # Send a ping every 60 seconds
        except Exception as e:
            logger.error(f"Error while sending keep-alive: {str(e)}")
            await asyncio.sleep(60)  # Retry after 60 seconds

# Run the keep-alive in the background
asyncio.create_task(keep_alive(conn))
