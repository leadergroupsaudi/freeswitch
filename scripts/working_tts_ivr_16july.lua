...
if not ivrfile then
    freeswitch.consoleLog("norice",call_uuid.." Error: Unable to open IVR file.\n")
    return
end
...
if not webConfigfile then
    freeswitch.consoleLog("norice",call_uuid.."Error: Unable to open webConfigfile file.\n")
    return
end
...
if not extensionConfigfile then
    freeswitch.consoleLog("norice",call_uuid.."Error: Unable to open ExtensionConfigfile file.\n")
    return
end
...
if not recordTypeConfigfile then
    freeswitch.consoleLog("norice",call_uuid.."Error: Unable to open recordTypeConfigfile file.\n")
    return
end
...
--local dynamicApi = require "resources.functions.dynamicApi"
        local function find_childNode(data)
                        ...
                        freeswitch.consoleLog("notice",call_uuid.."Found IVR Node ID: " .. iVRChildNodeId\n)
        end

        local function find_childNode_with_dtmfinput(digits,data)
                freeswitch.consoleLog("notice",call_uuid.."*************** Find Child Node With DTMF Digit input : " ..digits\n);
                ...
                freeswitch.consoleLog("notice",call_uuid.."Found IVR Child Node ID: " .. iVRChildNodeId\n)
                if iVRChildNodeId == 0 then
                        freeswitch.consoleLog("notice",call_uuid.."IVR Child Node ID is not found with DTMF: " .. digits .. " and Parant Node ID: " .. data.NodeId\n)
                        freeswitch.consoleLog("NOTICE",call_uuid.."Hangup the Call\n")
                        session:hangup()
                end
        end
...
-- Function to insert spaces between digits in a string
	...
	local function setLanguage(languageCode)
		freeswitch.consoleLog("INFO",call_uuid.."Language Set Function\n")
	...
	local function operation_code_108_exec(data)
		...
		--cmd = "user_exists id "..extension.." "..domain
                --found = api:executeString(cmd)
                --freeswitch.consoleLog("notice",call_uuid..": Extension Output result"..found\n)
                --if found == 'true' then
		if extension ~= nil then
                   freeswitch.consoleLog("notice",call_uuid..": Extesnion found\n")
		   dialString = "sofia/gateway/fxax-gateway/"..extension
	           second_session = freeswitch.Session(dialString)
                   if (second_session:ready()) then
                      freeswitch.consoleLog("WARNING",call_uuid..": second leg answered\n")
                      freeswitch.bridge(session, second_session)
                      freeswitch.consoleLog("WARNING",call_uuid..": After bridge\n")
                   else
                      freeswitch.consoleLog("WARNING",call_uuid..": second leg failed\n")
	      	   end
		else	   
		   freeswitch.consoleLog("NOTICE",call_uuid..": Extension Not Found\n")
		   session:set_tts_parms("flite", "awb");
		   session:speak("The pre defined Extension is not valid");
		   inputKey = "F"
		   find_childNode_with_dtmfinput(inputKey,data)
		...
		freeswitch.consoleLog("NOTICE",call_uuid.."API ID: "..api_id\n)
		...
		--freeswitch.consoleLog("INFO",call_uuid.. "Method:: "..methodType.." :: contentType :: "..contentType.." :: serviceURL ::"..serviceURL.." :: apiInput ::".. json.encode(apiInputdata))
		local finalApi = constructApi(methodType,contentType,serviceURL,apiInputdata)
		freeswitch.consoleLog("NOTICE",call_uuid.."Final modified API: "..finalApi\n)
	   if contentType == "multipart/form-data" then
		   local apiResponse = execute_command(finalApi)
		   freeswitch.consoleLog("INFO",call_uuid.."HTTP response:"..apiResponse\n)
                   local endIndex = string.find(apiResponse, '+')
                   local curl_response_code = tonumber(string.sub(apiResponse, endIndex + 1))
                if curl_response_code >=200 and curl_response_code < 300 then
                       local curl_response = string.sub(apiResponse, 1, endIndex-1)
		       freeswitch.consoleLog("INFO",call_uuid.."HTTP response:"..curl_response\n)
                        if #apiOutput > 2 then
                            freeswitch.consoleLog("INFO",call_uuid.."API Output: "..apiOutput\n)
                            apiOutput = json.decode(apiOutput)
                            freeswitch.consoleLog("INFO",call_uuid.."Curl Response: "..curl_response\n)
                            curl_response = json.decode(curl_response)
                            for _,key in ipairs(apiOutput) do
                                if key.ParentResultId == nil then
                                        freeswitch.consoleLog("INFO",call_uuid.."Found NULL ParentID\n")
                                        session:setVariable(key.ResultFieldTag,json.encode(curl_response[key.ResultFieldName]))
                                end
                            end
                            for _,key in ipairs(apiOutput) do
                                if key.ParentResultId ~= nil then
                                        freeswitch.consoleLog("INFO",call_uuid.."Found ParentID for ResultFieldTag"..key.ResultFieldTag\n)
                                        local parentResult = json.decode(session:getVariable(key.ParentResultId))
                                        session:setVariable(key.ResultFieldTag,parentResult[key.ResultFieldName])
                                end
                            end
                         ...
                         freeswitch.consoleLog("INFO",call_uuid.."Null apiOutput\n")
                         inputKeys = "S"
                        end
                else
                        inputKeys = "F"
		...
		session:execute("curl", finalApi)
		--freeswitch.consoleLog("INFO",call_uuid.. tostring(api_response)\n)
		curl_response_code = tonumber(session:getVariable("curl_response_code"))
		curl_response      = session:getVariable("curl_response_data")
		freeswitch.consoleLog("NOTICE",call_uuid.."Curl Response Code: "..curl_response_code\n)
		if curl_response then
			freeswitch.consoleLog("NOTICE",call_uuid.."Curl Response Data: "..curl_response\n)
		end
		if curl_response_code >=200 and curl_response_code < 300 then
                        if #apiOutput > 2 then
                            freeswitch.consoleLog("INFO",call_uuid.."API Output: "..apiOutput\n)
		...
		local finalApi = constructApi(methodType,contentType,serviceURL,apiInputdata)
		freeswitch.consoleLog("NOTICE","Final modified API: "call_uuid.. ..finalApi\n)
		session:execute("curl", finalApi)
		--freeswitch.consoleLog("INFO",call_uuid..  tostring(api_response)\n)
		curl_response_code = tonumber(session:getVariable("curl_response_code"))
		curl_response      = session:getVariable("curl_response_data")
		freeswitch.consoleLog("NOTICE","Curl Response Code: "call_uuid.. ..curl_response_code\n)
		if curl_response then
			freeswitch.consoleLog("NOTICE","Curl Response Data: "call_uuid.. ..curl_response\n)
		end
		if curl_response_code >=200 and curl_response_code < 300 then
			inputKeys = "S"
		else
	...
	local function operation_code_120_exec(data)
	 	for _,childNode in pairs(data.ChildNodeConfig) do
		        if childNode.ApplyComparison == true then
		                if childNode.ComparisonOperator == "GRT" then
						 ...
						 if  tonumber(session:getVariable(childNode.CollectionTag)) > tonumber(childNode.Value1) then freeswitch.consoleLog("INFO","Found ChildNodeId: "call_uuid.. ..childNode.ChildNodeId) Sub_menu(childNode.ChildNodeId\n) break
						 end
			                 else 	
						 if tonumber(childNode.CollectionTag) > tonumber(childNode.Value1) then freeswitch.consoleLog("Found ChildNodeId: "call_uuid.. ..childNode.ChildNodeId) Sub_menu(childNode.ChildNodeId\n) break 
						 end
					 end
                		elseif childNode.ComparisonOperator == "LST" then
					if childNode.OperandType == "T" then
		                        	if tonumber(session:getVariable(childNode.CollectionTag)) < tonumber(childNode.Value1) then freeswitch.consoleLog("INFO",call_uuid.."Found ChildNodeId: "..childNode.ChildNodeId) Sub_menu(childNode.ChildNodeId\n) break 
						end
					else 
						if tonumber(childNode.CollectionTag) > tonumber(childNode.Value1) then freeswitch.consoleLog("Found ChildNodeId: " call_uuid.. ..childNode.ChildNodeId) Sub_menu(childNode.ChildNodeId\n) break	
						end
				...
				elseif childNode.ComparisonOperator == "IBW" then
                                         if childNode.OperandType == "T" then
                                                 if  tonumber(session:getVariable(childNode.CollectionTag)) >= tonumber(childNode.Value1) and tonumber(session:getVariable(childNode.CollectionTag)) <= tonumber(childNode.Value2) then freeswitch.consoleLog("INFO","Found ChildNodeId: " call_uuid.. ..childNode.ChildNodeId) Sub_menu(childNode.ChildNodeId) break
                                                 end
                                         else 
						 if tonumber(childNode.CollectionTag) >= tonumber(childNode.Value1) and tonumber(childNode.CollectionTag) <= tonumber(childNode.Value2) then freeswitch.consoleLog("Found ChildNodeId: " call_uuid.. ..childNode.ChildNodeId) Sub_menu(childNode.ChildNodeId) break
                                                 end
                                         end
				end
		        else
                		freeswitch.consoleLog("INFO",call_uuid.."ApplyComparison:"..tostring(childNode.ApplyComparison\n))
		                freeswitch.consoleLog("INFO",call_uuid.."ChildNodeId: "..childNode.ChildNodeId\n) 
		...
		if data.InputType == 40 then
			session:setVariable("DefultInput",audioFile)
                ...
                freeswitch.consoleLog("INFO",call_uuid.."Audio file to convert into Text: " .. audioFile\n)
		local languageCode = session:getVariable("LanguageCode")
                freeswitch.consoleLog("INFO",call_uuid.."Language Code:: " .. languageCode\n)
                local api_id = 20
                local methodType,contentType,serviceURL, apiInputdata,inputKeys
                for _,api in pairs(webApiData) do
                        if api.apiId == api_id then
			freeswitch.consoleLog("INFO",call_uuid.."::Found API ID::\n")
                        methodType = api.methodType
                        contentType = api.inputMediaType
                        serviceURL = api.serviceURL
                        apiInputdata = json.decode(api.apiInput)
	...
	local function operation_code_330_exec(data) -- Text to Speech BuiltIn
		...
		freeswitch.consoleLog("INFO",call_uuid.."Text received to convert into Speech:: "..tts_text\n)
        ...
        local function execute_operation(nodedata)
                freeswitch.consoleLog("notice",call_uuid.."Operation Code Received: " .. nodedata.OperationCode\n)
                if (nodedata.OperationCode == 10) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing OperationCode 10\n")
		...
		elseif (nodedata.OperationCode == 11) then
                ...
                elseif (nodedata.OperationCode == 20) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing OperationCode 20\n")
                        operation_code_20_exec(nodedata);
                elseif (nodedata.OperationCode == 30) then
                        if nodedata.ValidKeys then
                                freeswitch.consoleLog("notice",call_uuid.."Executing OperationCode 30\n")
                                operation_code_30_exec(nodedata)
                        else
                                freeswitch.consoleLog("notice",call_uuid.."IVR Node Does not have Valid Keys" .. nodedata.NodeId\n)
                                session:hangup()
                        end
                elseif (nodedata.OperationCode == 31) then
                        if nodedata.ValidKeys then
                                freeswitch.consoleLog("notice",call_uuid.."Executing OperationCode 31\n")
                ...
                elseif (nodedata.OperationCode == 40) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing OperationCode 40\n")
                        operation_code_40_exec(nodedata);
                elseif (nodedata.OperationCode == 100) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 100\n")
                        operation_code_100_exec(nodedata)
                elseif (nodedata.OperationCode == 101) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 101\n")
                        operation_code_101_exec(nodedata)
                elseif (nodedata.OperationCode == 105) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 105\n")
                        operation_code_105_exec(nodedata)
                elseif (nodedata.OperationCode == 107) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 107\n")
                        operation_code_107_exec(nodedata)
                elseif (nodedata.OperationCode == 108) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 108\n")
                        operation_code_108_exec(nodedata)  
                elseif (nodedata.OperationCode == 111) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 111\n")
                        operation_code_111_exec(nodedata)
                elseif (nodedata.OperationCode == 112) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 112\n")
                        operation_code_112_exec(nodedata)
                elseif (nodedata.OperationCode == 120) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 120\n")
                        operation_code_120_exec(nodedata)
                elseif (nodedata.OperationCode == 200) then
                        freeswitch.consoleLog("notice",call_uuid.."Terminating the Call\n")
                        session:hangup()
                elseif (nodedata.OperationCode == 330) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 330\n")
                        operation_code_330_exec(nodedata)
                elseif (nodedata.OperationCode == 331) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 331\n")
                        operation_code_331_exec(nodedata)			
                elseif (nodedata.OperationCode == 341) then
                        freeswitch.consoleLog("notice",call_uuid.."Executing Opeartion Code 341\n")
                        operation_code_341_exec(nodedata)			
                else    freeswitch.consoleLog("err",call_uuid.."========== Operation Code " .. nodedata.OperationCode .. " is not Configured in service =================\n")
                        session:hangup();
                end
        end
        ...
        function Sub_menu(nodeid)
                ...
                freeswitch.consoleLog("notice",call_uuid.."Sub Menu IVR Child Node ID: " .. nodeid\n)
		  for key, value in pairs(ivrdata) do
	...
	if session:answered() and session:getVariable("cc_last_nodeId") ~= nil then
		freeswitch.consoleLog("NOTICE",call_uuid.."************* Agent Evaluation *******************"\n)
		...
		if cc_cancel_reason == "TIMEOUT" then 
			freeswitch.consoleLog("NOTICE",call_uuid.."******** cc_cancel_reason :: "..cc_cancel_reason\n)
		...
		if cc_agent_bridged == "true" then freeswitch.consoleLog("NOTICE",call_uuid.."******** cc_agent_bridged :: "..cc_agent_bridged\n)
		...
		freeswitch.consoleLog("notice",call_uuid.. cc_agent_extension .. " Extension Status : " .. extension_status\n)
		if extension_status ~= ext_not_reg  then
			...
			freeswitch.consoleLog("notice",call_uuid.."Extension " .. cc_agent_extension .. " Not registered . Changing status to Logged Out ..\n")
			api:executeString("callcenter_config agent set status " .. cc_agent_extension .. " 'Logged Out'")
		end
		freeswitch.consoleLog("NOTICE",call_uuid..":Agent Evaluation Node ID: "..cc_nodeId\n)
                  for key, value in pairs(ivrdata) do
                    if value.NodeId == tonumber(cc_nodeId) then
                     iVRChildNodeId = value.ChildNodeConfig[1].ChildNodeId;
		     freeswitch.consoleLog("NOTICE",call_uuid.."child Node ID: "..iVRChildNodeId\n)
                     Sub_menu(iVRChildNodeId)
                     break
                    end
		  end
		end
		--freeswitch.consoleLog("NOTICE",call_uuid..": Child Node ID Not found\n")
		session:hangup()
	end
	...
	session:execute("sleep","500")
        freeswitch.consoleLog("notice",call_uuid..": Session Answered\n")
                ...
                for _, node in pairs(ivrdata) do
                	    ...
                	    freeswitch.consoleLog("notice",call_uuid.."...Start Menu found... IVR Node ID:" ..node.NodeId.. "  IVR Node Name: " ..node.NodeName.. " Operation Code: " ..node.OperationCode\n);
