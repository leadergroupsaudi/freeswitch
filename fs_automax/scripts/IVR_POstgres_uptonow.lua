...
-- SQL to create the agentstack table if it doesn't exist
...
if not success then
    freeswitch.consoleLog("ERR", "Failed to create the agentstack table - Error: " .. tostring(err) .. "\n")
    return
end

freeswitch.consoleLog("INFO", "Table agentstack is ready.\n")
...
if not extension_data or not extension_data.Extensions then
    ...
    for _, extension in ipairs(extension_data.Extensions) do
        local extensionCode = extension.ExtensionCode  -- Use the correct key
        local isAgent = extension.IsAgent or false      -- Use the correct key
        local priority = extension.PriorityOrder or 0   -- Use the correct key
        local jsonData = json.encode(extension)         -- Convert the whole extension data to JSON if needed

        -- Insert only if IsAgent is false (considered inactive)
        if not isAgent then
            -- Prepare SQL statement to insert the extension along with its status
            local sql = string.format([[
                INSERT INTO agentstack (extensioncode, isactive, incall, noofcalls, talktime, waittime, priority, json_data)
                VALUES ('%s', FALSE, FALSE, 0, 0, 0, %d, '%s')
                ON CONFLICT (extensioncode) DO NOTHING;  -- Do nothing if the extension already exists
            ]], extensionCode, priority, jsonData)
