...
local ivrFilePath = scripts_path.."/ivr-cc-config/IVRConfiguration_KFF.json"
...
local extensionsFilePath = scripts_path.."/ivr-cc-config/Extension_KFF.json"
...
--local ivrrecording_data = redis:eval("return redis.call('json.get','RecordingType_qa');", 0)
local recordTypeFilePath = scripts_path.."/ivr-cc-config/RecordingType_KFF.json"
...
-- Function to insert spaces between digits in a string
    ...
    -- Monitor agent call and transfer when the agent hangs up
--    local bridge_uuid = session:getVariable("cc_agent_uuid")
  --    freeswitch.consoleLog("notice", "No agent was bridged.")
   -- if bridge_uuid then
     --  freeswitch.consoleLog("notice", "Monitoring agent call with UUID: " .. bridge_uuid)
        session:execute("callcenter","ccm-ivr@default")
        
        -- Wait for the agent to hang up, then transfer
       -- while session:ready() do
          --  session:sleep(1000)
        --end
        
        -- Once the agent ends the call, transfer to dialplan context
        -- Example: Transfer to extension 3600 in the "default" context
        --session:execute("transfer", "OPCODE_101_KFF XML public")
    --else
      --  freeswitch.consoleLog("notice", "No agent was bridged.")
   -- end
...
--               session:execute("sleep", "2000")  -- Adjust the sleep time as needed
                --session:execute("sleep","500")
               -- session:setAutoHangup(false)
               -- session:setVariable("cc_last_nodeId",data.NodeId)
                --session:execute("transfer","OPCODE_101  XML public")
    -- Transfer to the next step or extension after ensuring the call center has handled the call properly
  --           session:execute("transfer", "OPCODE_101_KFF XML public")
--		 freeswitch.consoleLog("notice", "\nTransferring the call to OPCODE_101_KFF in the dialplan\n")
  --                session:transfer("OPCODE_101_KFF", "XML", "public")
	--	 session:execute("sleep","500")
         --       session:setAutoHangup(false)
           --     session:setVariable("cc_last_nodeId",data.NodeId)
             --   session:execute("transfer","OPCODE_101_KFF  XML public")

	--	session:execute("transfer", "1302 XML public")
--		session:setAutoHangup(false)
  --             session:setVariable("cc_last_nodeId",data.NodeId)
--             session:execute("transfer","OPCODE_101_KFF  XML public")
	...
	local function operation_code_101_exec(data)
		...
		session:setVariable("cc_last_nodeId",data.NodeId)
		session:execute("transfer","OPCODE_101  XML public")
	--	session:execute("transfer","1320  XML default")
	       -- if session:ready() then
  --      freeswitch.consoleLog("notice", "Transferring call to user 1320 after agent call ends.")
    --    session:execute("transfer", "1320 XML default")
   -- end
	end
