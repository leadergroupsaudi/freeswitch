...
-- Function to insert spaces between digits in a string
	...
	local function operation_code_101_exec(data)
		 ...
		 local agent_status_header = session:getVariable("sip_h_X-Agent-Status")
		freeswitch.consoleLog("notice", "X-Agent-Status: " .. agent_status_header .. "\n")
		--local is_dnd_mode = (agent_status_header == "DND")
	--	freeswitch.consoleLog("notice", "is_dnd_mode: " .. tostring(is_dnd_mode) .. "\n")
                local extensions = {}
                local ext_not_reg = "error/user_not_registered"
                for key, value in pairs(agent_extensions.Extensions) do
                        if value.IsAgent == false then
					  ...
					  local agent_state = api:executeString("callcenter_config agent get state " ..value.ExtensionCode)
					     ...
					     if agent_state == "In a queue call" then
						   	...
						   	if agent_status_header == 'On Break' then
                        					freeswitch.consoleLog("notice", "Setting Agent " .. value.ExtensionCode .. " status to DND as per header.\n")
                        					api:executeString("callcenter_config agent set status " .. value.ExtensionCode .. " Break")
                   					 else	
                                        			api:executeString("callcenter_config agent set status " .. value.ExtensionCode .. " Available")
                                        			api:executeString("callcenter_config agent set contact " .. value.ExtensionCode .. " " .. extension_status);
								api:executeString("callcenter_config agent set state " .. value.ExtensionCode .. " Waiting")
