...
def send_audio_recording_to_axionic_server(event, new_token):
    try:
        ...
        compressed_file_path = trimmed_file_path + '.gz'
        with open(trimmed_file_path, 'rb') as f_in:
            with gzip.open(compressed_file_path, 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
        
        # Prepare the compressed file for upload
        files = [('File', (file_name + '.gz', open(compressed_file_path, 'rb'), 'application/gzip'))]
       ...
       #         os.remove(compressed_file_path)  # Delete the compressed file after successful upload
        ...
        # Close the file handle to avoid resource leaks
