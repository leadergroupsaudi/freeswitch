...
session:setVariable("Caller_Id", caller_id)
...
local generalSettings = ivrconfig.GeneralSettingValues
local availabilityScheduleJson = ivrconfig.IVRConfiguration[1].GeneralSettingValues[6].SettingValue
local availabilitySchedule = json.decode(availabilityScheduleJson)

local WebConfigFilePath =scripts_path.."/ivr-cc-config/webAPIConfig_fortunil.json"
...
-- Function to insert spaces between digits in a string
	...
	function execute_command(command)
	    freeswitch.consoleLog("INFO","in execute_command Function")
	    freeswitch.consoleLog("INFO", "Executing command: " .. command .. "\n")
            local handle = io.popen(command)
            local result = handle:read("*a")
            handle:close()
	    if not result or result == "" then
		    freeswitch.consoleLog("WARNING", "Command executed but no output was returned.\n")
	    else
		    freeswitch.consoleLog("INFO", "Command output: " .. result .. "\n")
            end
            return result
        end
				...
				if item.InputValueType == "D" or item.InputValueType == "E" then
				 ...
				 local inputValue;
                                 if item.InputValueType == "D" or item.InputValueType == "E" then
                                        freeswitch.consoleLog("INFO","Inout Value :: ".. item.InputValue)
                                        inputValue = session:getVariable(item.InputValue)
					freeswitch.consoleLog("INFO","input Value: "..inputValue)
				 elseif item.InputValueType == "S" then 
					inputValue = item.InputValue
                                 end
				 freeswitch.consoleLog("INFO", "Checking inputValue for "..item.InputType..": "..inputValue)
				 if string.find(inputValue, "%.wav$") then
                                        formData = formData.." -F "..item.FieldName.."="..inputValue
					freeswitch.consoleLog("INFO","In if ")
                                 else
                                        formData = formData.." -F "..item.FieldName.."="..inputValue
					  freeswitch.consoleLog("INFO","In else ")
                                 end
	...
	local function getRecordingDuration(wavFilePath)
    		local cmd = string.format("sox --i -D %s 2>&1", wavFilePath)
    ...
    -- Convert the result to a number and return it
    	...
    	return 0 -- Return 0 if no valid duration is found
	end
	...
	local function operation_code_40_exec(data)
		...
		local recordingTypeId = data.RecordingTypeId;
		 -- Log incoming data
    		freeswitch.consoleLog("info", "Data received: " .. json.encode(data))
    		freeswitch.consoleLog("info", "InputTimeLimit: " .. tostring(data.InputTimeLimit))
    		freeswitch.consoleLog("info", "RecordingTypeId: " .. tostring(recordingTypeId))

		for _,recordData in pairs(recording_config.RecordingType) do
			freeswitch.consoleLog("info", "RecordingTypeId: " .. tostring(recordData.RecordingTypeId))
    			freeswitch.consoleLog("info", "TypePrefix: " .. tostring(recordData.TypePrefix))
			freeswitch.consoleLog("info", "Checking recordData.RecordingTypeId: " .. tostring(recordData.RecordingTypeId))
    			freeswitch.consoleLog("info", "Against recordingTypeId: " .. tostring(recordingTypeId))
		...
		session:setVariable("C_FileName", recordfilename)
		session:execute("export", "nolocal:C_FileName=" .. recordfilename)
		...
		local recording_duration = getRecordingDuration(recording_filename)
    		freeswitch.consoleLog("NOTICE", "Recording Duration: " .. recording_duration .. " seconds")
		session:setVariable("C_RecordingDuration", tostring(recording_duration))
    		session:execute("export", "nolocal:C_RecordingDuration=" .. tostring(recording_duration))
	...
	local function operation_code_108_exec(data)
		...
		for _,api in pairs(webApiData) do
			...
			freeswitch.consoleLog("NOTICE", "Method Type: " .. tostring(methodType) .. "\n")
    			freeswitch.consoleLog("NOTICE", "Content Type: " .. tostring(contentType) .. "\n")
    			freeswitch.consoleLog("NOTICE", "Service URL: " .. tostring(serviceURL) .. "\n")
    			freeswitch.consoleLog("NOTICE", "API Input Data: " .. tostring(json.encode(apiInputdata)) .. "\n")
    			freeswitch.consoleLog("NOTICE", "API Output: " .. tostring(apiOutput) .. "\n")
			break
			end
		end
	   ...
	   if contentType == "multipart/form-data" then
		   freeswitch.consoleLog("INFO","Entered form data part")
		   freeswitch.consoleLog("NOTICE", "Final API: " .. tostring(finalApi) .. "\n")
		   local apiResponse = execute_command(finalApi)
	...
	local function operation_code_330_exec(data) -- Text to Speech BuiltIn
	   ...
	   local function convert_to_24hr_minutes(time_str)
    -- Extract hour, minute, and period (AM/PM)
    		time_str = string.gsub(time_str, "^%s*(.-)%s*$", "%1")
                local hour, minute, period = string.match(time_str, "(%d+):(%d+)(%a+)")
                hour = tonumber(hour)
                minute = tonumber(minute)

    -- Convert to 24-hour format
                if period == "PM" and hour ~= 12 then
                hour = hour + 12  -- Convert PM times to 24-hour format
                elseif period == "AM" and hour == 12 then
                hour = 0  -- Handle 12 AM as 00:00 in 24-hour format
                end

    -- Return the time in total minutes since midnight
                return hour * 60 + minute
	end


	-- Function to check if the office is available based on the schedule
	local function is_office_available()
    		local availabilityScheduleJson = ivrconfig.IVRConfiguration[1].GeneralSettingValues[6].SettingValue
    		local availabilitySchedule = json.decode(availabilityScheduleJson)
                 freeswitch.consoleLog("INFO","Entered in to office_available function")
    -- Get current day and time
    		local currentDay = os.date("%a"):upper()  -- Example: "Mon", "Tue"
    		local currentTime = os.date("%I:%M%p")  -- Example: "09:00AM"
		 freeswitch.consoleLog("INFO","Current Day:: "..currentDay)
		   freeswitch.consoleLog("INFO","Current Time:: "..currentTime)
		 local currentUTCDateTime = os.date("!%Y-%m-%dT%H:%M:%SZ") -- Format: "Thu Dec  4 15:19:47 2024"
		 --local currentUTCDateTime = os.date("!%Y-%m-%dT%H:%M:%SZ")
		--local hour, minute = string.match(currentUTCDateTime, "(%d+)-(%d+)")
		--print(currentUTCDateTime)
		freeswitch.consoleLog("INFO", "Current UTC Date and Time: " .. currentUTCDateTime)
		session:setVariable("C_DateTime", currentUTCDateTime)
		session:execute("export", "nolocal:C_DateTime=" .. currentUTCDateTime)
		freeswitch.consoleLog("INFO", "C_DateTime set to: " .. currentUTCDateTime)
		 local currentMinutes = convert_to_24hr_minutes(currentTime)
		 -- Special handling for Friday and Saturday
--    if currentDay == "FRI" or currentDay == "SAT" then
    	if  currentDay == "SAT" then
        freeswitch.consoleLog("INFO", "It's " .. currentDay .. ". Office is closed.")
        return false  -- Always route to IVR on Friday and Saturday
    end

    -- Check availability based on the schedule
    for _, schedule in ipairs(availabilitySchedule) do
        freeswitch.consoleLog("INFO", "In for loop")

        local sch_day = schedule.Day
        freeswitch.consoleLog("INFO", "Schedule Day: " .. sch_day)

        -- Check if the current day matches the schedule day
        if schedule.Day == currentDay then
            freeswitch.consoleLog("INFO", "Schedule found for today")

            local scheduleFrom = schedule.Schedule.From
            local scheduleTo = schedule.Schedule.To
	    --     freeswitch.consoleLog("INFO", "Schedule From: " .. scheduleFrom)
            -- freeswitch.consoleLog("INFO", "Schedule To: " .. scheduleTo)

            -- If both From and To are set
            if scheduleFrom ~= "" and scheduleTo ~= "" then
                -- Convert schedule times to total minutes since midnight
                local fromMinutes = convert_to_24hr_minutes(scheduleFrom)
                local toMinutes = convert_to_24hr_minutes(scheduleTo)

                -- Print schedule times in minutes for debugging
                freeswitch.consoleLog("INFO", "Schedule From: " .. scheduleFrom .. " (" .. fromMinutes .. " minutes)")
                freeswitch.consoleLog("INFO", "Schedule To: " .. scheduleTo .. " (" .. toMinutes .. " minutes)")

                -- If current time is within the available range
                if currentMinutes >= fromMinutes and currentMinutes <= toMinutes then
                    freeswitch.consoleLog("INFO", "Office is available.")
                    return true  -- Office is available
                else
                    freeswitch.consoleLog("INFO", "Current time is outside the office hours.")
                end
            else
                freeswitch.consoleLog("INFO", "No schedule for today.")
            end
        end
    end

    -- If no match or not within schedule range
    freeswitch.consoleLog("INFO", "Office is closed.")
    return false  -- Office is closed
end
		...
		elseif (nodedata.OperationCode == 11) then
                        freeswitch.consoleLog("notice","Executing OperationCode 11")
...
if session:ready() then
	...
	freeswitch.consoleLog("INFO", "The generated callLogId is: " .. callLogId)
	...
	session:execute("sleep","500")
		...
		local officeAvailable = is_office_available()
		if officeAvailable then
    -- Office is available, so proceed with the normal IVR flow
    ...
    -- Office is not available, go to the "Office Closed" message
    for _, node in pairs(ivrdata) do
        if node.NodeId == 3900 then  -- NodeId for "Office Closed Message" is 3900
            freeswitch.consoleLog("notice", "...Office Closed Menu found... IVR Node ID:" .. node.NodeId .. "  IVR Node Name: " .. node.NodeName .. " Operation Code: " .. node.OperationCode)
            execute_operation(node)
            break
        end
    end
