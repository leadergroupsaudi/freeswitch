...
login_credentials = {"username":"HtUztAaXLEVTOINyW+I3Fw==","password":"HtUztAaXLEVTOINyW+I3Fw=="}
url = 'https://v2.automaxsw.com/stage/automax2/CCM/api/IVR/Authenticate'
callLog_update_api = "https://v2.automaxsw.com/Stage/Axionic/CCM/API/CLK/CallLog/update"
get_callLog_api= "https://v2.automaxsw.com/Stage/Axionic/CCM/API/CLK/CallLog/Id"
callLog_addparticipant_api= "https://v2.automaxsw.com/Stage/Axionic/CCM/API/CLK/CallLog/AddParticipants"
...
def send_audio_recording_to_axionic_server(event,new_token):
	try:
		...
		payload={'CallLogId': callLogID, 'RecordingType': "A",'StartedDate':record_start_time, 'Duration':record_duration}
...
def update_callLog_data(payload, new_token):
	...
	if token_header:
		...
		response = requests.post(callLog_update_api,data=payload,headers=token_header)
		if response.status_code == 200:
			data = response.json()
...
def add_callLog_participant(payload, new_token):
	logger.debug("Entered into add_callLog_participant block")
	logger.debug("CallLog payload Data is {}".format(payload))
	token_header = get_token(new_token)
	if token_header:
		token_header['Content-Type'] = 'application/json'
		response = requests.post(callLog_addparticipant_api,data=payload,headers=token_header)
		if response.status_code == 200:
			data = response.json()
...
async def channel_answer_event_handler(event):
	...
	try:
		...
		add_payload = json.dumps([{ "CallLogId": int(callLogID), "CallerId": agent_extension, "JoinTime": call_created_time, "IsCaller": False, "answerStatus": "A", "CallReachTime": call_answered_time}])
		if token is None:
			new_token=True
		else:
			new_token = False
		add_callLog_participant(add_payload,new_token)
	except Exception as e:
