...
call_record_upload_api = "https://modhousestaging14.fortunil.com/api/method/axionic_integration.axionic_integration.api.voip.UpdateRecordingChildTable"
callLog_addparticipant_api ="https://modhousestaging14.fortunil.com/api/method/axionic_integration.axionic_integration.api.voip.CreateOrUpdateCallLog"
callLog_update_api ="https://modhousestaging14.fortunil.com/api/method/axionic_integration.axionic_integration.api.voip.CreateOrUpdateCallLog"
...
def send_audio_recording_to_axionic_server(event):
    try:
        ...
        logger.debug("Current event: {}".format({key: event[key] for key in event}))
        ...
        if "variable_record_start_time" in event:
            ...
            logger.debug(f"start_time: {record_start_time}")
...
async def channel_answer_event_handler(event):
        ...
        try:
                callLogID = event.get("variable_sip_h_X-CallLogId")
                agent_extension = event.get("Caller-Callee-ID-Number") or event.get("Caller-Destination-Number") or event.get("variable_sip_to_user")
                ...
                logger.debug(f"Duration: {raw_duration}")
                ...
                call_answered_time = call_answered_time.strftime("%Y-%m-%dT%H:%M:%S.") + f"{call_answered_time.microsecond:06d}Z"
                #call_answered_time = call_answered_time.strftime("%Y-%m-%dT%H:%M:%SZ")
                logger.debug("Before End Time")
                #end_time = start_time_dt + timedelta(seconds=duration)
...
async def hangup_event_handler(event):
	...
	if (val == "CALL_REJECTED" and event.get("Call-Direction") == "outbound") or\
	   (val == "USER_BUSY" and event.get("Call-Direction") == "outbound"): 
		...
		call_type = "Answered"
	elif term_code in ['486','603','480'] or val == "ALLOTTED_TIMEOUT":
		call_type = "Missed"
		term_flag = True
		call_start_time = event.get("variable_start_stamp")
	...
	if term_flag or term_code in ['486','603','480']or val == "NO_ANSWER"or val == "ALLOTTED_TIMEOUT":
		...
		logger.debug(f"agent_extension: {agent_extension}")
    ...
    "participants": [
        {
            "CallerId": agent_extension,
            "JoinTime": call_created_time,
            "HangupTime": end_time,  # Assuming HangupTime is the same as JoinTime for now
	...
	try:
		if ("variable_sip_h_X-CallLogId" in event and event.get("Call-Direction") == "inbound"):
			...
			call_end_time	= event.get("variable_end_stamp")
			logger.debug(f"call_from: {call_from}")
			started		= datetime.strptime(event.get("variable_start_stamp"), "%Y-%m-%d %H:%M:%S")
			ended		= datetime.strptime(call_end_time, "%Y-%m-%d %H:%M:%S")			
