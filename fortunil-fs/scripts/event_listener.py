...
async def channel_answer_event_handler(event):
    ...
    try:
        ...
        logger.debug(f"Global storage after update: {global_storage}")
        logger.debug(f"CHANNEL_BRIDGE callLogID: {callLogID}")
        payload = {
            "participants": [
                {
...
async def hangup_event_handler(event):
    ...
    logger.debug(f"hangup value : {val}")
    dial_status = event.get("variable_DIALSTATUS")
    logger.debug(f"dial_status : {dial_status}")
    term_flag = False
    ...
    if (val == "CALL_REJECTED" and event.get("Call-Direction") == "outbound") or\
        ...
        call_type = "UserBusy"
        logger.debug(f"call_type : {call_type}")
        ...
        call_type = "Answered"
    elif term_code in ['486', '603'] or val == "NO_ANSWER" or val == "ALLOTTED_TIMEOUT":
        call_type = "Missed"
        term_flag = True
        logger.debug(f"call_type : {call_type}")
        call_start_time = event.get("variable_start_stamp")
    elif val == "NO_USER_RESPONSE":  # Handling Rejected Case
        call_type = "Rejected"
        term_flag = True
        logger.debug(f"call_type : {call_type}")
        call_start_time = event.get("variable_start_stamp")
    ...
    if term_flag or term_code in ['486', '603', '480'] or val == "NO_ANSWER" or val =="ALLOTTED_TIMEOUT":
        ...
        # payload = [{ "CallerId": agent_extension, "JoinTime": call_created_time, "HangupTime": call_created_time,"AnswerStatus": "M","IsCaller": "0", "CallReachTime": call_start_time}]
         # Setting AnswerStatus based on call_type
        answer_status = "M"  # Default to Missed
        if call_type == "Rejected":
            answer_status = "R"  # Rejected case
        payload = {
            "participants": [
                {
                    "CallerId": agent_extension,
                    ...
                    "AnswerStatus": answer_status,  # Replace with a dynamic status if available
                    "IsCaller": "0",  # Indicates the participant is not the caller
                    "CallReachTime": call_created_time
                }
            ]
    ...
    try:
        if ("variable_sip_h_X-CallLogId" in event and event.get("Call-Direction") == "inbound"):
            ...
            callLogID = event.get("variable_sip_h_X-CallLogId")
            logger.debug("X-Call Log ID is {}".format(callLogID))
            logger.debug(f"calltype : {call_type}")
            ...
            agent_end = stored_data.get("agent_end_time")
            #agent_extension = event.get("Caller-Username") or event.get("variable_sip_to_user")
            agent_extension = event.get("Caller-Callee-ID-Number") or event.get ("variable_destination_number")
            logger.debug(f"call type is : {call_type}")
            payload = {
