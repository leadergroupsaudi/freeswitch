...
async def channel_create_handler(event):
    try:
        ...
        # Check if this is a callcenter queue channel
        queue_name = event.get("variable_callcenter_queue")
        assigned_agent = event.get("variable_cc_agent")  # agent assigned in callcenter

        if assigned_agent:
            logger.debug(f"[CHANNEL_CREATE] Agent assigned: {assigned_agent}")
            callee = assigned_agent  # send notification to actual agent
            call_type = "queue"
        elif queue_name:
            logger.debug(f"[CHANNEL_CREATE] Queue channel detected: {queue_name}")
            callee = queue_name
            call_type = "queue"

        # Build payload
        payload = {
            "domain": event.get("variable_sip_domain_name", "pbx.axionic.io"),
            "port": event.get("variable_sip_port", "7440"),
            "callerId": caller,
            "receivers": [callee] if callee else [],
            "mediaType": "audio",
            "callType": call_type,
            "isScreenShared": False
        }

        logger.debug(f"[CHANNEL_CREATE] Sending call notification: {payload}")

        # Send via FS bgapi
        fs_conn = connect_to_fs()
        cmd = f'bgapi curl https://wapis.discretal.com/calls/sip-call-notification content-type application/JSON post "{json.dumps(payload)}" ssl-verifyhost 0 ssl-verifypeer 0'
        logger.debug(f"[CHANNEL_CREATE] Running FS command: {cmd}")
        fs_conn.api(cmd)
        fs_conn.disconnect()

    except Exception as e:
        logger.debug(f"Error in channel_create_handler: {e}")
