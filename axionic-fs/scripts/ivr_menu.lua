...
local ivrconfig = json.decode(redis:eval("return redis.call('json.get','IVRConfiguration_qa');", 0)) -- storing the IVRNodes data
...
local audiopath = "/usr/local/freeswitch-ivr-instance/share/freeswitch/sounds/ivr_audioFiles/"
local recording_dir = '/usr/local/freeswitch-ivr-instance/recordings/IVR-CCM-Recordings/'
local module_folder = "/usr/local/freeswitch-ivr-instance/share/freeswitch/scripts/"
...
--local dynamicApi = require "resources.functions.dynamicApi"
        ...
        local function operation_code_10_exec(data)
	...
	local function operation_code_40_exec(data)
		...
		--session:execute("playback_terminators", "#")
		session:execute("export","playback_terminators=#")
	...
	local function operation_code_120_exec(data)
        ...
        local function execute_operation(nodedata)
                ...
                if (nodedata.OperationCode == 10) then
                        ...
                        operation_code_10_exec(nodedata)
...
if session:ready() then
	...
	--session:setVariable("sip_h_X-CallLogId","744")
        session:answer()
	session:execute("sleep","500")
        freeswitch.consoleLog("notice","Session Answered")
                ...
                for key, value in pairs(ivrdata) do
                        if(value.IsStartNode == true) then
                	    freeswitch.consoleLog("notice","...Start Menu found... IVR Node ID:" ..value.NodeId.. "  IVR Node Name: " ..value.NodeName.. " Operation Code: " ..value.OperationCode);
			    execute_operation(value)
			    break
                     end
