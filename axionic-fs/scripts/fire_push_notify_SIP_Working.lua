...
--	  if presence_status ~= nil and (presence_status == "In call" or presence_status == "Unregistered" or presence_status == "Invalid") then
  --          freeswitch.consoleLog("INFO","The call not going to connected because of presence_status is")
    --        session:answer()
...
-- if isFromAxionic and isFromAxionic:lower() == "true" then
  ...
  --         else
   ...
   freeswitch.consoleLog("NOTICE", "API Response: " .. tostring(userPresence_data) .. "\n")
		 ...
		 --freeswitch.consoleLog("NOTICE", "API Response: " .. json.encode(userPresence_data) .. "\n")
    --    end
        
--	if userPresence_data ~= nil then
	  local errorMessage = tostring(userPresence_data)
    --                    local errorMessage = userPresence_data.message
               if errorMessage == "In call" or errorMessage == "Unregistered" or errorMessage =="Invalid" then
		        freeswitch.consoleLog("INFO","Entering in status ")
                      --    freeswitch.consoleLog("NOTICE","Extension: " "\nPresence: " .. user.errorMessage)
