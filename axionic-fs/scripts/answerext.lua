...
local orig_caller =tostring(argv[3])
local  destination =tostring(argv[4])
...
--        return nil
	...
	--        freeswitch.consoleLog("INFO", string.format("[createCallLog] Phone: %s | Extension: %s | Call Using: %s\n", phone, extension, call_using))
        ...
        if tonumber(response_code) >= 200 and tonumber(response_code) < 300 then
			...
			cmd="uuid_setvar      "..tostring(uuid).."      ".."callLogId".."             "..tostring(callLogId);
	   ...
	   session:setVariable("orig_caller", orig_caller)
        session:execute("export", "nolocal:orig_caller=" .. orig_caller)
        session:setVariable("sip_h_X-orig_caller", orig_caller)
        session:execute("export", "nolocal:sip_h_X-orig_caller=" .. orig_caller)	    
	
	  session:setVariable(" destination", destination)
        session:execute("export", "nolocal:destination=" .. destination)
        session:setVariable("sip_h_X-destination",  destination)
        session:execute("export", "nolocal:sip_h_X-destination=" ..  destination)

	freeswitch.consoleLog("NOTICE", "Already processed callLogId: " .. orig_caller .. "\n")
	return callLogId
                end
            else
