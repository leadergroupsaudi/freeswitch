		...
		for _, item in ipairs(inputArray) do
			...
			elseif item.InputType == "R" then
    			...
    			--	freeswitch.consoleLog("INFO", "Processing InputType: " .. item.InputType .. " with InputValueType: " .. item.InputValueType)
			        freeswitch.consoleLog("INFO", "Processing InputType: " .. tostring(item.InputType) .. " with InputValueType: " .. tostring(item.InputValueType))
				if item.InputValueType == "D" or item.InputValueType == "E" then
					 freeswitch.consoleLog("INFO", "IF Part: " .. tostring(item.InputType) .. " with InputValueType: " .. tostring(item.InputValueType))
				--	freeswitch.consoleLog("INFO", "IF Part: " .. item.InputType .. " with InputValueType: " .. item.InputValueType)
					...
					local finalValue = tostring(rawInput):gsub("{{" .. variableName .. "}}", val)
                			freeswitch.consoleLog("INFO", "Fetched dynamic value for " .. tostring(key) .. ": " .. tostring(finalValue))
                			payload[key] = finalValue
        			else
					 freeswitch.consoleLog("ERR", "InputValueType is D or E. No dynamic value found for " .. tostring(key))
...
--				value = type(value) == "table" and json.encode(value):gsub('"', '\\"') or tostring(value)
		...
		--		local classificationEnValue = session:getVariable("ClassificationIdEn")
				local classificationEnValue = session:getVariable("ClassificationIdEn") or session:getVariable("ClassificationIdAr")
				if classificationValue then
   				 payload["Classification"] = classificationValue
    				freeswitch.consoleLog("INFO", "Updated payload ClassificationEn to: " .. classificationValue .. "\n")
				else
	...
	local function operation_code_101_exec(data)
                ...
                for key, value in pairs(agent_extensions.Extensions) do
					...
					-- Check agent's DND status
                			  ...
                			  freeswitch.consoleLog("notice", "Agent " .. value.ExtensionCode .. " DND Status: " .. (dnd_status or "Not Set") .. "\n")
                                        --api:executeString("callcenter_config agent add " .. value.ExtensionCode .. " Callback")
					  local agent_state = api:executeString("callcenter_config agent get state " ..value.ExtensionCode)
                                           freeswitch.consoleLog("notice", "Agent " .. value.ExtensionCode .. " State: " .. agent_state .. "\n")
					   if dnd_status == "Busy" then
                    				freeswitch.consoleLog("notice", "Agent " .. value.ExtensionCode .. " is in DND. Skipping this agent.\n")
					   else
						local agent_state = api:executeString("callcenter_config agent get state " .. value.ExtensionCode)
                    				freeswitch.consoleLog("notice", "Agent " .. value.ExtensionCode .. " State: " .. agent_state .. "\n")
					    if agent_state == "In a queue call" then
                				freeswitch.consoleLog("notice", "Agent " .. value.ExtensionCode .. " is currently in a queue call. Skipping this agent.\n")
						else	
                                        	api:executeString("callcenter_config agent set status " .. value.ExtensionCode .. " Available")
                                        	api:executeString("callcenter_config agent set contact " .. value.ExtensionCode .. " " .. extension_status);
						api:executeString("callcenter_config agent set state " .. value.ExtensionCode .. " Waiting")
...
--	local IncidentDetailsTextEn = session:getVariable("IncidentDetailsTextEn") or ""
	...
	local map_value = json.encode({coordinates = {0, 0}}) 
    ...
    { name = "Map", value =  map_value}, -- Proper JSON string with quotes
    { name = "Location", value = LocationIdEn },
    { name = "District", value = LocationIdEn }, -- You can change if district differs
    { name = "Status", value = "Open" },
    { name = "Attachments", value = attachmentID }
}
}
    ...
    "-H 'Authorization: %s' " ..  -- %s will insert Bearer <token> without quotes
    "-d '%s'",
    url,
    token,
    payload)
		...
		--end		   
         ...
         --[[       if curl_response_code >=200 and curl_response_code < 300 then
			...
			if recordID and recordID ~= "" and attachment_id and attachment_id ~= "" then
       			 freeswitch.consoleLog("INFO", "Calling update_incident_with_attachment with recordID: " .. recordID .. " and attachment_id: " .. attachment_id .. "\n")
       			 update_incident_with_attachment(recordID, attachment_id)
    			else
        		freeswitch.consoleLog("INFO", "Skipping update_incident_with_attachment, missing recordID or attachment_id\n")
    			end
        		inputKeys = "S"
    		else
		...
		if curl_response_code >=200 and curl_response_code < 300 then
                ...
                -- Try to access the value
					...
					session:setVariable(key.ResultFieldTag, json.encode(value))
					 ...
					 access_token = access_token:gsub('^"(.*)"$', '%1')
					 local authToken = "Bearer " .. access_token 
					 local getclassificationurl = "curl -s -X GET 'https://automax.discretal.com/api/classifications/hierarchy?client=mobile' -H 'Authorization: " .. authToken .. "'"
                                        local handle = io.popen(getclassificationurl)
                                        local getClassificationResponse = handle:read("*a")
                                        handle:close()
                                        freeswitch.consoleLog("INFO", "Classification Hierarchy Response: " .. getClassificationResponse .. "\n")
				--	local getlocationurl ="curl -s -X GET 'https://automax.discretal.com/api/locations'  -H 'Authorization: " .. authToken .. "'"
					local getlocationurl = "curl -s -X GET \"https://automax.discretal.com/api/locations\" -H \"Authorization: Bearer " .. access_token .. "\""
					...
					local classification_index = tonumber(session:getVariable("ClassificationIdEn") or session:getVariable("ClassificationIdAr"))
					local subclassification_index = tonumber(session:getVariable("SubClassificationIdEn") or session:getVariable("SubClassificationIdAr"))
					local location_index = tonumber(session:getVariable("LocationIdEn") or session:getVariable("LocationIdAr"))
					freeswitch.consoleLog("INFO", "[DEBUG] classification_index: " .. tostring(classification_index) .. "\n")
					freeswitch.consoleLog("INFO", "[DEBUG] subclassification_index: " .. tostring(subclassification_index) .. "\n")
					freeswitch.consoleLog("INFO", "[DEBUG] location_index: " .. tostring(location_index) .. "\n")
					freeswitch.consoleLog("INFO", "[DEBUG] getClassificationResponse: " .. tostring(getClassificationResponse:sub(1, 500)) .. "\n")
					--  Call your function to get the classification ID
					local final_classification_id, err = getClassificationID(getClassificationResponse, classification_index, subclassification_index)
					...
					--  Set session variable or log error
						if final_classification_id then
    							...
    							freeswitch.consoleLog("INFO", "Mapped ClassificationEn to ID: " .. final_classification_id .. "\n")
						else
    							freeswitch.consoleLog("ERR", "Error getting classification ID: " .. (err or "unknown error") .. "\n")
						end
                        		freeswitch.consoleLog("INFO", "Set Variable: " .. key.ResultFieldTag .. " = " .. json.encode(value))
							if final_location_id then
    								session:setVariable("LocationIdEn", final_location_id)
    								freeswitch.consoleLog("INFO", "Mapped LocationIdEn to ID: " .. final_location_id .. "\n")
							else
    								freeswitch.consoleLog("ERR", "Error getting location ID: " .. (err or "unknown error") .. "\n")
							end
                    			else
  ...
  --              session:execute("speak", "ssml")
	...
	--	 if char ~= " " then
        ...
        for _, wav_file in ipairs(split_numbers) do
            local file_path = string.format("/usr/local/freeswitch-automax-instance/share/freeswitch/sounds/%s/%s", tag_name,wav_file)
