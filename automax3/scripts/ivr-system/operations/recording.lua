...
function M.execute(operation_code, node_data)
    logger:info(string.format("Executing recording operation %d for node %d", operation_code, node_data.NodeId))
...
local function has_sound_activity(wav_file_path)
    ...
    local cmd = string.format("sox %s -n stat 2>&1 | grep -i 'RMS     amplitude' | awk '{print $3}'",
    ...
    if result and tonumber(result) then
        ...
        logger:debug(string.format("Sox RMS amplitude: %f (threshold: %f)", amplitude, threshold))
...
function M.record_message(node_data)
    logger:info(string.format("Operation 40: Recording message for node %d", node_data.NodeId))
...
-- Print values
freeswitch.consoleLog("INFO", "Recording Config: " .. tostring(recording_config) .. "\n")
freeswitch.consoleLog("INFO", "Recording Type ID: " .. tostring(recording_type_id) .. "\n")
...
--if not record_filename_prefix then
local id = tonumber(recording_type_id)

if id == 22 then
    record_filename_prefix = "IVR_CALLEE_NAME_RECORDING"
elseif id == 23 then
    record_filename_prefix = "IVR_INC_DESCRIPTION_RECORDING"
else
    record_filename_prefix = "IVR_DEFAULT_RECORDING"
end
    -- Build recording filename
    local record_filename = string.format("%s_%s.wav", record_filename_prefix, call_uuid)
    ...
    logger:info(string.format("Recording config - TimeLimit: %d sec, Silence: %d sec, File: %s", record_time_limit,
        silence_hits, recording_path))

    -- Set playback terminators
    session:execute("export", "nolocal:playback_terminators=#")

    -- Start recording
    local record_cmd = string.format("%s %d %d %d", recording_path, record_time_limit, silence_threshold, silence_hits)
    ...
    if has_sound_activity(recording_path) then
        ...
        if node_data.TagName then
            ...
            logger:debug(string.format("Stored recording path in variable: %s = %s", node_data.TagName, recording_path))
        end
...
function M.speech_to_text(node_data)
    logger:info(string.format("Operation 341: Speech-to-text for node %d", node_data.NodeId))
    ...
    -- Get language code
    local language_code = session_manager.get_variable("TTSLanguageCode") or "en-US"
    ...
    local access_token = session_manager.get_variable("azure_subscription_key")
    ...
    if content_type == "audio/wav" then
        curl_cmd = string.format("curl -s -w '+%%{http_code}' -X %s -H 'Ocp-Apim-Subscription-Key: %s'" ..
                                     " -H 'Content-Type: %s' --data-binary @%s '%s'", method_type or "POST",
            access_token or "", content_type, string_utils.shell_escape(audio_file),
            service_url .. "?language=" .. language_code)
    else
    ...
    logger:info("STT API command: " .. curl_cmd)
    ...
    if end_index then
        ...
        if response_code and response_code >= 200 and response_code < 300 then
            ...
            local success, parsed = json.decode(response_body)
            if success and parsed then
                local encode_success, parsed_json = json_utils.encode(parsed)
                if encode_success then
                    logger:info("STT API parsed response: " .. parsed_json)
                end
            else
                logger:error("Failed to parse STT response: " .. tostring(parsed))
            end
            if success and parsed then
                ...
                if general_settings then
                    for _, settings in pairs(general_settings) do
                        if settings.SettingId == 14 then
                            ...
                            local success, stt_settings = json_utils.decode(settings.SettingValue)
                            if not success then
                                logger:error(string.format("Failed to parse stt settings JSON: %s",
                                    tostring(stt_settings)))
                            end
                            local encode_success, stt_json = json_utils.encode(stt_settings)
                            if encode_success then
                                logger:info("STT Settings: " .. stt_json)
                            else
                                logger:error("Failed to encode STT settings: " .. tostring(stt_json))
                            end
                            -- Store the STT DisplayText result in session variables
                            if parsed and parsed.DisplayText then
                                local display_text = parsed.DisplayText

                                -- Store in the configured STT response variable
                                if stt_settings and stt_settings.TextResponseFieldTag then
                                    session_manager.set_variable(stt_settings.TextResponseFieldTag, display_text)
                                    logger:info("Stored STT result in " .. stt_settings.TextResponseFieldTag .. ": " .. display_text)
                                end

                                -- Also store in node TagName if specified
                                if node_data.TagName then
                                    session_manager.set_variable(node_data.TagName, display_text)
                                    logger:info("Stored STT result in " .. node_data.TagName .. ": " .. display_text)
                                end
                            else
                                logger:warning("STT response missing DisplayText field")
                            end
                            break
                        end
                    end
