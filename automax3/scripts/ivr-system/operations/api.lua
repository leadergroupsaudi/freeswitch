...
function api_operation_113(session, data)
    freeswitch.consoleLog("INFO", "[operations.api] === OPERATION 113: WhatsApp Submit ===\n")
...
local function map_indices_to_uuids(session)
    ...
    -- Classification: priority ClassificationId2En → ClassificationId2Ar → ClassificationIdEn → ClassificationIdAr
    local class_id2en = tonumber(session:getVariable("ClassificationId2En"))
    local class_id2ar = tonumber(session:getVariable("ClassificationId2Ar"))
    local class_id1en = tonumber(session:getVariable("ClassificationIdEn"))
    local class_id1ar = tonumber(session:getVariable("ClassificationIdAr"))

    local final_class_index
    local used_class_id2 = false
    if class_id2en then
        final_class_index = class_id2en + 9
        used_class_id2 = true
    elseif class_id2ar then
        final_class_index = class_id2ar + 9
        used_class_id2 = true
    elseif class_id1en then
        final_class_index = class_id1en
    elseif class_id1ar then
        final_class_index = class_id1ar
    end

    local subclass_index = tonumber(session:getVariable("SubClassificationIdEn") or
                                        session:getVariable("SubClassificationIdAr"))

    -- Location: priority LocationId3En → LocationId3Ar → LocationId2En → LocationId2Ar → LocationIdEn → LocationIdAr
    local location_priority = {{
        en = "LocationId3En",
        ar = "LocationId3Ar",
        offset = 18,
        level = 3
    ...
    local final_location_index

    for _, entry in ipairs(location_priority) do
        local val = tonumber(session:getVariable(entry.en)) or tonumber(session:getVariable(entry.ar))
        if val then
            final_location_index = val + entry.offset
            break
        end
    end

    logger:info(string.format("Mapping indices - Classification: %s, SubClass: %s, Location: %s",
        tostring(final_class_index), tostring(subclass_index), tostring(final_location_index)))

    -- Fetch classifications from tree endpoint (hierarchical structure with children)
    local raw = fetch_api_data("https://epmstg.automaxsw.com/api/v1/admin/classifications/tree", access_token)
    local classifications = {}

    if raw then
        -- Filter only IVR type classifications (skip "Demo" type "both")
        for _, item in ipairs(raw) do
            if item.types then
                for _, t in ipairs(item.types) do
                    if t == "ivr" then
                        table.insert(classifications, item)
        ...
        logger:info(string.format("Fetched %d IVR classification(s)", #classifications))

        -- Log full classification tree: all parents and their children
        for pi, parent in ipairs(classifications) do
            if parent.children and #parent.children > 0 then
                logger:info(string.format("[CLASSIFICATION_API] [%d] Parent: %s (UUID: %s) | children: %d", pi,
                    parent.name or "unnamed", parent.id or "nil", #parent.children))
                for ci, child in ipairs(parent.children) do
                    logger:info(string.format("[CLASSIFICATION_API]   [%d.%d] Child: %s (UUID: %s)", pi, ci,
                        child.name or "unnamed", child.id or "nil"))
                end
            else
                logger:info(string.format("[CLASSIFICATION_API] [%d] Parent: %s (UUID: %s) | no children", pi,
                    parent.name or "unnamed", parent.id or "nil"))
            end
        end

        local class_id

        if final_class_index then
            logger:info(string.format("Mapping DTMF input %s as child index under first IVR parent",
                tostring(final_class_index)))
            class_id = get_classification_id_by_index(classifications, 1, final_class_index)
            if class_id then
                logger:info("Mapped to UUID: " .. class_id)
                local parent = classifications[1]
                local child = parent and parent.children and parent.children[final_class_index]
                logger:info(string.format("[CLASSIFICATION_SELECTED] Parent: %s | SubClass: %s | UUID: %s",
                    parent and parent.name or "unknown", child and child.name or "unknown", class_id))
            else
                logger:error("Failed to map classification index to UUID")
            end
        else
            logger:info("No index set - using fallback (first available)")
            local first_parent = classifications[1] -- "Ministry"
            if first_parent then
                if first_parent.children and #first_parent.children > 0 then
                    class_id = first_parent.children[1].id
                    logger:info(string.format("Fallback: Using first child: %s (%s)",
                        first_parent.children[1].name or "unnamed", class_id))
                else
                    class_id = first_parent.id
                    logger:info(string.format("Fallback: No children, using parent: %s (%s)",
                        first_parent.name or "unnamed", class_id))
                end
            else
                logger:error("No IVR classifications found")
            end
        end

        if class_id then
            session:setVariable("SelectedClassificationIdEn", class_id)
            session:setVariable("SelectedClassificationIdAr", class_id)
            if used_class_id2 then
                session:setVariable("SelectedClassificationId2En", class_id)
                session:setVariable("SelectedClassificationId2Ar", class_id)
            end
            logger:info("==> FINAL: SelectedClassificationIdEn/Ar = " .. class_id)
        else
            logger:error("==> FINAL: class_id is nil, nothing set")
        end
    else
        logger:error("Failed to fetch or empty response from /tree endpoint")
    end
    -- Fetch location tree and find "india" parent node (mirrors classification tree approach)
    logger:info("Fetching location tree from API: https://epmstg.automaxsw.com/api/v1/admin/locations/tree")
    local location_tree = fetch_api_data("https://epmstg.automaxsw.com/api/v1/admin/locations/tree", access_token)
    local india_node = nil

    if location_tree then
        -- Find the "india" node by code
        -- local india_node = nil
        for _, node in ipairs(location_tree) do
            if node.code == "saudi_arabia" then
                india_node = node
                break
            end
        end
        ...
        if india_node then
            local loc_children = india_node.children or {}
            logger:info(string.format("[LOCATION_API] Found parent: %s (UUID: %s) | children: %d",
                india_node.name or "unnamed", india_node.id or "nil", #loc_children))

            -- Log all children of india
            for ci, child in ipairs(loc_children) do
                logger:info(string.format("[LOCATION_API]   [%d] Child: %s (UUID: %s)", ci, child.name or "unnamed",
                    child.id or "nil"))
            end

            local loc_id
            if final_location_index then
                logger:info(string.format("Location index IS SET - mapping index %s to child UUID",
                    tostring(final_location_index)))
                loc_id = get_location_id_by_index(loc_children, final_location_index)
                if loc_id then
                    local matched = loc_children[final_location_index]
                    logger:info(string.format("[LOCATION_SELECTED] Parent: %s | Child: %s | UUID: %s",
                        india_node.name or "unknown", matched and matched.name or "unknown", loc_id))
                else
                    logger:error("Failed to map location index to UUID")
                end
            else
                -- No index set - use first child of india
                logger:info("Location index NOT SET - using fallback (first child of india)")
                if loc_children[1] and loc_children[1].id then
                    loc_id = loc_children[1].id
                    logger:info(string.format("Fallback: Using first child: %s (%s)", loc_children[1].name or "unnamed",
                        loc_id))
                else
                    logger:error("Fallback failed: No children found under india node")
                end
            end
            ...
            if loc_id then
                ...
                session:setVariable("SelectedLocationIdEn", loc_id)
                session:setVariable("SelectedLocationIdAr", loc_id)
            ...
            logger:error("[LOCATION_API] 'india' node not found in location tree")
        end
    else
        logger:error("Failed to fetch location tree from API")
    end

    -- Set Title of Incident
    if classifications and india_node then
        if final_class_index and classifications[1].children[final_class_index] then
            logger:info(string.format("Using classification index %s (%s) for title mapping",
                tostring(final_class_index), classifications[1].children[final_class_index].name or "unknown"))
        else
            logger:info("No classification index set for title mapping, using fallback")
        end
        local loc_name_en = india_node.children and india_node.children[final_location_index] and
                                india_node.children[final_location_index].name or india_node.name or "Unknown Location"
        local loc_name_ar = loc_name_en
        local incident_title_en = "IVR Incident - " ..
                                      (classifications[1].children[final_class_index] and classifications[1].children[final_class_index].name or
                                          "Unknown") .. " at " .. loc_name_en
        local incident_title_ar = "حادث - " ..
                                      (classifications[1].children[final_class_index] and classifications[1].children[final_class_index].name or
                                          "غير معروف") .. " في " .. loc_name_ar
    ...
    -- Fetch Workflow
    logger:info("Fetching workflow from API: https://epmstg.automaxsw.com/api/v1/admin/workflows?record_type=incident")
    local workflows = fetch_api_data("https://epmstg.automaxsw.com/api/v1/admin/workflows?record_type=incident",
        access_token)
    if workflows then
    ...
    for _, item in ipairs(input_array) do
        ...
        elseif input_type == "R" then
            ...
            if item.InputValueType == "D" or item.InputValueType == "E" then
                ...
                if not val and item.DefaultValue then
                    ...
                    logger:info(string.format("  [REQUEST BODY] Variable nil, using DefaultValue for '%s' = '%s'", key,
                        val))
                end
            else
                ...
                if val then
                    ...
                    elseif key == "text_body" then
                        ...
                        payload["text"] = {
                            preview_url = false,
                            body = tostring(val):gsub('^"(.*)"$', '%1')
                        }
                        logger:info(string.format("  [REQUEST BODY] Built WhatsApp text object, body = %s",
                            tostring(val)))
                    else
                        val = tostring(val):gsub('^"(.*)"$', '%1')
                        payload[key] = val
        ...
        elseif input_type == "F" then
            ...
            if input_value and string.find(input_value, "%.wav$") then
                --form_data = form_data .. " -F " .. item.FieldName .. "=@" .. input_value .. ";type=audio/x-wav"
                form_data = form_data .. " -F '" .. item.FieldName .. "=@" .. input_value .. ";type=audio/x-wav'"

                logger:info(string.format("  [FORM_DATA] Added file field: %s = @%s;type=audio/wav", item.FieldName, input_value))                                    
    ...
    elseif content_type == "audio/wav" then
        ...
        if next(payload) ~= nil then
            ...
            elseif content_type == "application/json" then
                -- Priority: SelectedClassificationId2En → SelectedClassificationId2Ar → SelectedClassificationIdEn → SelectedClassificationIdAr
                local classification_value
                for _, key in ipairs({"SelectedClassificationId2En", "SelectedClassificationId2Ar",
                                      "SelectedClassificationIdEn", "SelectedClassificationIdAr"}) do
...
local function perform_reauth(session)
    ...
    local auth_curl = construct_api(auth_api.methodType, auth_api.inputMediaType, auth_api.serviceURL, api_input_data,
        auth_api.apiType)
                    ...
                    if type(decoded) == "table" then
                        ...
                        if decoded.data then
                            if decoded.data.id then
                                session:setVariable("complaint_number_response",
                                    decoded.data.complaint_number or decoded.data.id)
                                session:setVariable("complaint_id_response",
                                    decoded.data.complaint_number or decoded.data.id)
                                -- if api_id == 16 or api_id == 24 then
                                -- logger:info("[INCIDENT_CREATE] Set complaint_number = " .. tostring(decoded.data.complaint_number))
                                session:setVariable("complaint_id",
                                    decoded.data.complaint_number or decoded.data.record_id)
                                -- end
                            ...
                            if (api_id == 11 or api_id == 12) then
                                ...
                                local incident_number = decoded.data.incident_number or decoded.data.record_number or
                                                            decoded.data.recordNumber or decoded.data.number or
                                                            decoded.data.id

                                if incident_number then
                                    session:setVariable("incident_no_reponse", tostring(incident_number))
                                    logger:info("[INCIDENT_CREATE] Set incident_no_reponse = " ..
                                                    tostring(incident_number))
                                else
                                    ...
                                    logger:info("[INCIDENT_CREATE] Available data fields: " ..
                                                    table.concat(data_keys, ", "))
                                end
                            end
                        end
