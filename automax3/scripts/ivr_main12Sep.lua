...
freeswitch.consoleLog("INFO", "Scripts path: " .. tostring(scripts_path) .. "\n")
local ivrFilePath = scripts_path .. "/ivr-cc-config/ivrconfig.json"

-- local record_file = scripts_path .. "/call_recording/" .. call_uuid .. ".wav"

-- Start recording immediately
-- session:recordFile(record_file)
...
-- Improved execute_command: runs shell command and removes any temporary file marker
function execute_command(command)
    if not command then
        return nil
    end
    -- detect cleanup marker syntax: __TMP__:/full/path
    local tmpfile = command:match("__TMP__:(%S+)")
    if tmpfile then
        -- strip the marker from the actual command
        command = command:gsub("%s*__TMP__:%S+", "")
    end

    freeswitch.consoleLog("INFO", "execute_command: " .. command .. "\n")
    ...
    if handle then
        ...
        freeswitch.consoleLog("ERR", "execute_command: failed to popen command\n")
    end
    ...
    if tmpfile then
        ...
        if not ok then
            freeswitch.consoleLog("ERR", "execute_command: failed to remove tmpfile: " .. tostring(err) .. "\n")
        else
            freeswitch.consoleLog("INFO", "execute_command: removed tmpfile: " .. tmpfile .. "\n")
        end
    end
...
local function constructApi(methodType, contentType, serviceURL, apiInputdata)
    ...
    for _, item in ipairs(inputArray) do
            ...
            if item.InputValueType == "D" or item.InputValueType == "E" then
                ...
                if variableName then
                    ...
                    if val then
                        ...
                        freeswitch.consoleLog("INFO", "Fetched dynamic value for " .. tostring(key) .. ": " ..
                            tostring(finalValue))
                        payload[key] = finalValue
                    else
                        freeswitch.consoleLog("ERR", "No dynamic value found for " .. tostring(key))
                    end
                end
            else
                ...
                if key and val then
                    if key == "Map" then
                        ...
                        local cleanVal = tostring(val):gsub('^"(.*)"$', '%1')
        ...
        elseif item.InputType == "B" then
            if item.InputValueType == "D" or item.InputValueType == "E" then
                ...
                binaryData = binaryData .. " --data-binary @" .. item.InputValue
            end
        elseif item.InputType == "F" then
            ...
            if string.find(inputValue, "%.wav$") then
                ...
                formData = formData .. " -F " .. item.FieldName .. "=" .. shell_escape(inputValue)
            end
    ...
    local finalApi = serviceURL .. " -s -w '+%{http_code}' "

    if contentType == "multipart/form-data" then
        local headers_clean = string.gsub(headers, '"', '')
        finalApi =
            "curl -s -w '+%{http_code}' -X " .. methodType .. " -H '" .. headers_clean .. "' " .. formData .. " " ..
                finalApi
    elseif contentType == "audio/wav" then
        local headers_clean = string.gsub(headers, '"', '')
        finalApi =
            "curl -s -w '+%{http_code}' -X " .. methodType .. " -H '" .. headers_clean .. "' -H 'Content-Type: " ..
        ...
        if next(payload) ~= nil then
            ...
            -- Use shell_escape for the payload
            finalApi = finalApi .. " -X " .. methodType .. " -d " .. shell_escape(encodedPayload)
        end

        if #headers > 0 then
            local headers_clean = string.gsub(headers, '"', '')
            finalApi = finalApi .. " -H \"" .. headers_clean .. "\""
        end
    end
...
local function operation_code_341_exec(data) -- Speech to Text
    ...
    local api_id = 20
    local methodType, contentType, serviceURL, apiInputdata, inputKeys, apiOutput
    ...
    local apiResponse = execute_command(finalApi)
    freeswitch.consoleLog("INFO", "STT API Response: " .. tostring(apiResponse))

    -- Parse response and handle accordingly
    local endIndex = string.find(apiResponse, '+')
    local curl_response_code = 500
    local curl_response = ""

    if endIndex then
        curl_response_code = tonumber(string.sub(apiResponse, endIndex + 1)) or 500
        curl_response = string.sub(apiResponse, 1, endIndex - 1)
    end

    local apicallResult = "F"
    if curl_response_code >= 200 and curl_response_code < 300 then
        if apiOutput and #apiOutput > 2 then
            local success, decoded_response = pcall(json.decode, curl_response)
            if success and decoded_response then
                local apiOutputTable = json.decode(apiOutput)
                for _, key in ipairs(apiOutputTable) do
                    if key.ParentResultId == nil then
    ...
    for _, settings in pairs(generalSettings) do
        if settings.SettingId == 14 then
            ...
            local textResponse = session:getVariable(stt_settingValue.TextResponseFieldTag)
            if textResponse then
                session:setVariable(data.TagName, textResponse)
            end
            break
        end
    end
...
function on_hangup(s, status, arg)
    local record_file = s:getVariable("call_recording_path")
    if record_file and record_file ~= "" then
        freeswitch.consoleLog("INFO", "Stopping recording: " .. record_file .. "\n")
        s:execute("stop_record_session", record_file)
    end
end
local function start_call_recording()
    ...
    if not session:ready() then
        freeswitch.consoleLog("WARN", "Session not ready for recording")
        return false
    end
    ...
    local recording_filename = "call_" .. call_uuid .. ".wav"
    local full_recording_path = recording_dir .. recording_filename
    
    -- Check media state before recording
--    local media_ready = session:getVariable("media_ready")
--    if media_ready ~= "true" then
 --       freeswitch.consoleLog("WARN", "Media not ready, skipping recording")
 --       return false
 --   end
        freeswitch.consoleLog("INFO", "Starting recording at " .. full_recording_path .. "\n")
    -- Start recording with error handling
    session:execute("record_session", full_recording_path)
--    local result = session:recordFile(full_recording_path, 600, 500, 3)
    -- if result then
        session:setVariable("call_recording_path", full_recording_path)
        session:setVariable("call_recording_filename", recording_filename)
        freeswitch.consoleLog("INFO", "Call recording started: " .. full_recording_path)
        return true
end


-- On hangup, upload file via curl
--[[ session:hangupHook(function(s, status)
    freeswitch.consoleLog("INFO", "Call ended, uploading recording...\n")
    local record_file = session:getVariable("call_recording_path")
    if record_file == "" then
        freeswitch.consoleLog("ERR", "No file path found for call recording!\n")
        return
    end
    ...
    -- Access token from session variable
    local access_token = session:getVariable("Access_token")
    if not access_token or access_token == "" then
        freeswitch.consoleLog("ERR", "No access token found in session!\n")
        return
    end

    -- Build curl command (capture output with -s)
    local cmd = string.format("curl -s -X POST '%s' " .. "-H 'Authorization: Bearer %s' " ..
                                  "-H 'Content-Type: multipart/form-data' " .. "-F 'fieldName=Attachments' " ..
                                  "-F 'upload=@%s' " .. "--compressed", http_url, access_token, record_file)
    ...
    freeswitch.consoleLog("INFO", "Upload API response: " .. tostring(response) .. "\n")

    -- Try to decode JSON response (make sure you have dkjson or cjson loaded)
    local ok, json = pcall(require, "dkjson")
    local attachment_id, record_id
    if ok and response and #response > 2 then
        local decoded = json.decode(response)
        if decoded then
            attachment_id = decoded.id or decoded.attachment_id
            -- record_id = decoded.recordID or decoded.record_id
        end
    end

    local record_id = session:getVariable("recordID")
    if record_id and attachment_id then
        -- session:setVariable("recordID", record_id)
        session:setVariable("attachment_id", attachment_id)
        update_incident_with_attachment(record_id, attachment_id)
    else
        freeswitch.consoleLog("NOTICE", "Attachment not saved or response invalid.\n")
    end
end)
]]
if session:ready() then
    ...
    if session:ready() then
        local recording_started = start_call_recording()
        if not recording_started then
            freeswitch.consoleLog("WARN", "Proceeding without call recording")
        end
    end
    ...
    local access_token = session:getVariable("Access_token")
    if not access_token or access_token == "" then
        freeswitch.consoleLog("ERR", "No access token found in after loop session!\n")
    end
...
-- Call ended → upload file
freeswitch.consoleLog("INFO", "Call ended, uploading recording...\n")

local record_file = session:getVariable("call_recording_path")
if not record_file or record_file == "" then
    freeswitch.consoleLog("ERR", "No file path found for call recording!\n")
else
    -- API URL
    local http_url = "https://automax.discretal.com/api/compose/namespace/432712349708976129/module/432712349708910593/record/attachment"
--        "https://automax.discretal.com/api/compose/namespace/425787942130548737/module/425787942127468545/record/attachment"

    -- Access token from session variable
-- local access_token = session:getVariable("Access_token")
freeswitch.consoleLog("INFO", session:getVariable("Access_token"))	
local access_token = "eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9..rG6GC-CzBTefgjdQwlsqlnyrGqb6x4XYP4UFVOX-6Qd-oajsFX0rq1C5RU7RCztdKacv9rXZr2teuzXdIZsqSA"
...
if success and decoded_response and decoded_response.response and decoded_response.response.attachmentID then
    local attachment_id = decoded_response.response.attachmentID
    local record_id = session:getVariable("recordID")

    freeswitch.consoleLog("INFO", "Decoded attachment ID: " .. tostring(attachment_id) .. "\n")
    ...
    freeswitch.consoleLog("ERR", "JSON decode failed or missing attachmentID. Raw: " .. tostring(curl_response) .. "\n")
end

-- Log final state
freeswitch.consoleLog("INFO", "Decode success: " .. tostring(success) .. "\n")
freeswitch.consoleLog("INFO", "Decoded table: " .. json.encode(decoded_response or {}) .. "\n")
...
--if ok and response and #response > 2 then
--            local decoded = json.decode(response)
--freeswitch.consoleLog("INFO", decoded.response.attachmentID or decoded.response)
--[[
local success, decoded_response = pcall(json.decode, curl_response)
            if success and decoded_response and decoded_response.response and decoded_response.response.attachmentID then
           freeswitch.consoleLog("INFO", decoded_response)
           -- if decoded then
                local attachment_id = decoded_response.response.attachmentID or decoded_response.response.attachment_id
                local record_id = session:getVariable("recordID")
                if record_id and attachment_id then
                    session:setVariable("attachment_id", attachment_id)
                    update_incident_with_attachment(record_id, attachment_id)
...
freeswitch.consoleLog("INFO", "success".. tostring(decoded_response) .. "\n")
freeswitch.consoleLog("INFO", .success)
]]
end
end
--[[
