...
local uuid       = tostring(argv[1] or session:getVariable("UUID"))
...
local answer_time = os.date("%Y-%m-%d %H:%M:%S")
api:executeString("uuid_setvar " .. uuid .. " answertime " .. answer_time)
log("INFO", string.format("uuid=%s callee=%s caller=%s answertime=%s", uuid, callee_num, caller_num, answer_time))
