...
local function upload_attachment(record_file, http_url, access_token)
    ...
    if code_num and code_num >= 200 and code_num < 300 then
        ...
        if success and decoded and decoded.response and decoded.response.attachmentID then
            return decoded.response.attachmentID, "SUCCESS"
        end
    end
    return nil, "UPLOAD_FAILED"
end

local function check_audio_file(file_path)
...
local function update_incident_with_attachments(recordID, attachment_ids)
    freeswitch.consoleLog("INFO", "Entering update_incident\n")

    -- Check input
    if not recordID or not attachment_ids or #attachment_ids == 0 then
        freeswitch.consoleLog("ERROR", "recordID or attachment_ids missing/empty\n")
        return
    end
    ...
    for _, att in ipairs(attachments_array) do
        table.insert(payload_table.values, att)
    end
