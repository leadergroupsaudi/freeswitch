...
--      session:execute("curl", callLog_api .. " content-type:application/json post '" .. useragent_json .. "'")
        session:execute("curl", "https://wapi.discretal.com/calls/addAxionicDetails content-type application/JSON post " .. useragent_json .. " ssl-verifyhost 0 ssl-verifypeer 0")
        ...
        if  tonumber(api_response_code) >= 200 and tonumber(api_response_code) <= 400 then
           ...
           if api_response_data then
		    local message = api_response_data.message or "No message"
                 ...
                 --   callLogId = math.floor(api_response_data.response.callLogId)
                 --      callLogId = api_response_data.response.call_id
                    -- callLogId = math.floor(api_response_data.response.call_id)
                    --freeswitch.consoleLog("NOTICE", "Call Log ID is ::"..callLogId.."\n")
