...
local from_user_name = api:executeString("user_data "..from_num.."@fsstg.axionic.io var effective_caller_id_name")
...
local userAuth_json = "{\"username\":\"freeswitch@leadergroup.com\",\"password\":\"LeaderSIP@123\"}"
session:execute("curl", "https://axionic.leadergroup.com/Staging/Identity/User/Authenticate content-type application/JSON post "..userAuth_json)
local userAuth_responseCode = session:getVariable("curl_response_code")
local userAuth_responseData = session:getVariable("curl_response_data")
local json_api_response = json.decode(userAuth_responseData)
local access_token = json_api_response.response.token
...
session:execute("curl", "https://axionic.leadergroup.com/Staging/AXBFF/V2/Administration/UserNotification/Call append_headers 'Authorization: Bearer "..access_token.."' content-type application/JSON post "..notify_json)
